<script lang="ts">
	...
	import MotionRender from '../canvas/MotionRender.svelte';
		...
		<div class="grid h-full w-full grid-cols-10">
			...
			<div class="col-span-6 self-center">
				...
				>
					...
					<button class="마우스" on:click={() => render.onChangeMotionState(MotionState.DEFAULT)}>
						<Icon name="mouse" />
					</button>
					<button class="손" on:click={() => render.onChangeMotionState(MotionState.DRAGGING)}>
						<Icon name="hand" />
					</button>
					...
					<button
						class="연필"
						on:click={() => () => render.onChangeMotionState(MotionState.DRAWING)}
					>
						<Icon name="pencil" />
					</button>
					<input
					...
					<button class="쓰레기통" on:click={controller.onDelete}>
						<Icon name="trash" />
					</button>
					<button class="저장" on:click={controller.onSaveWithButton}>
						<Icon name="save" />
					</button>
				</div>
			</div>
