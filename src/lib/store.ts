...
import { INITIAL_HSVA } from './constants';
...
const setInitalColor = () => {
  const { subscribe, set } = writable<HsvaColor>(INITIAL_HSVA);
  return {
    subscribe,
    set,
  }
}
const setMutableColor = () => {
  const { subscribe, set, update } = writable<HsvaColor>(INITIAL_HSVA);
  return {
    subscribe,
    set,
    update
...
const setPaletteBgColor = () => {
  const { subscribe, set, update } = writable<string>('#FFFF00');
  return {
    subscribe,
    set,
    update,
...
export const initialFillColor = setInitalColor();
export const initialStrokeColor = setInitalColor();
export const mutableColor = setMutableColor();
export const paletteBgColor = setPaletteBgColor();
